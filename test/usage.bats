...
JSONL
  ...
  # uv script tasks can emit dependency-install logs on stderr the first time a
  # new task runs. BATS merges stderr into $output, which would poison JSON
  # assertions. Warm this task once with output discarded.
  sessions usage "$SESSION_1" >/dev/null 2>&1
}
